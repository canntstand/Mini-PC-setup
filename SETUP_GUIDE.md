...
3. `ssh -L 8080:127.0.0.1:80 user@ip`
4. Перейти в браузере по `http://127.0.0.1:8080/uptime-kuma` в Uptime Kuma. Создать новый монитор с типом **Push** и задать ему имя `Home-Server`. Это необходимо, чтобы локальный контейнер `heartbeat_to_vps` смог считать его токен и начать ежеминутную отправку отчетов о жизнеспособности локального сервера.
5. Перейти в браузере по `http://127.0.0.1:8080/wg-easy` в WireGuard панель. Cоздать всех клиентов (в зависимости от того сколько устройств планируется подключить к VPN).
